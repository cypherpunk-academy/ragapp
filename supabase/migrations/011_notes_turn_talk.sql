-- =============================================================================
-- Notes: turn_id / talk_id — Herkunft aus Chat (Filo §5.1, §5.5).
-- Arbeitstext bleibt bestehen, auch wenn der Ursprungs-Chat gelöscht wurde
-- (turn_id/talk_id zeigen dann ins Leere — kein FK, rein informativ).
-- =============================================================================

ALTER TABLE app_notes ADD COLUMN IF NOT EXISTS turn_id text;
ALTER TABLE app_notes ADD COLUMN IF NOT EXISTS talk_id text;

CREATE INDEX IF NOT EXISTS idx_app_notes_turn_id ON app_notes (turn_id);
CREATE INDEX IF NOT EXISTS idx_app_notes_talk_id ON app_notes (talk_id);

-- =============================================================================
-- pull_changes — include turn_id/talk_id on notes
-- =============================================================================
...
-- =============================================================================
-- push_changes — persist turn_id/talk_id on notes
-- =============================================================================
...
BEGIN
  ...
  FOR rec IN SELECT * FROM jsonb_array_elements(
    ...
    INSERT INTO app_notes (
      id, user_id, paragraph_id, source_id, segment_slug, turn_id, talk_id,
  ...
  FOR rec IN SELECT * FROM jsonb_array_elements(
    ...
    UPDATE app_notes SET
      ...
      turn_id      = NULLIF(rec->>'turn_id', ''),
      talk_id      = NULLIF(rec->>'talk_id', ''),

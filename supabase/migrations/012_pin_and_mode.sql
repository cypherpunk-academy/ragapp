-- =============================================================================
-- rag_talks: pinned / mode — Welle 5a (Pin & Nacht-Cleanup) und 5c (Chat/Nachdenken-Modus).
-- pinned schuetzt einen Talk vor dem naechtlichen Cleanup unpinned Talks (>7 Tage,
-- ragrun app/main.py _talk_cleanup_loop). mode persistiert den zuletzt gewaehlten
-- Chat-Modus je Talk. Beide Spalten werden bereits von der ragrun-Alembic-Migration
-- 0024_add_pinned_and_mode_to_rag_talks angelegt; IF NOT EXISTS macht dies hier
-- idempotent, falls diese Migration vor der Alembic-Migration laeuft.
-- =============================================================================

ALTER TABLE rag_talks ADD COLUMN IF NOT EXISTS pinned boolean NOT NULL DEFAULT false;
ALTER TABLE rag_talks ADD COLUMN IF NOT EXISTS mode text NOT NULL DEFAULT 'chat';

-- =============================================================================
-- pull_changes — include pinned/mode on rag_talks
-- =============================================================================

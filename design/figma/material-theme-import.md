...
**Stand:** 2026-05-10  
**Source of Truth:** [`material-theme.json`](../tokens/material-theme.json) (Material Theme Builder Export, Seed `#184FC5`)  
**Ableitungen:** [`tokens.json`](../tokens/tokens.json) (Spacing, Radius, Typo) → `npm run build:theme`
...
Der ursprüngliche Plan in [`workflow.md`](./workflow.md) (Phase 2) sah ein eigenes Naming wie `color/brand/primary`, `color/text/primary` vor. Mit dem Wechsel zu **Material Theme Builder als Quelle** übernehmen wir das **M3-Schema direkt**:
...
- Tooling-kompatibel (MUI, M3 Web, Expo via `npm run build:theme`)
...
2. **Seed-Farbe eingeben:** `#184FC5`  
   *Optional:* alternativ `Import` → die Datei `design/tokens/material-theme.json` wählen, falls das Plugin Import unterstützt (manche Versionen ja, manche nein).
...
2. design/tokens/material-theme.json überschreiben
3. Bei Bedarf tokens.json anpassen
...
Werte in Figma Variables editieren und dieselben Werte in die passende JSON-Datei übernehmen, dann `npm run build:theme`. Siehe [`workflow.md`](./workflow.md).

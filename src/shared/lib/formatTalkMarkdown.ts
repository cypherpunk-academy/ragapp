...
function labelForHit(hit: RagHit): string {
  ...
  return parts.length > 0 ? parts.join(' — ') : (hit.snippet?.trim() || i18n.t('common.unknownSource'));
}
...
export function formatTalkAsMarkdown(
  ...
  for (const turn of turns) {
    if (turn.userMessage?.trim()) {
      lines.push(i18n.t('chat.exportYou', { message: turn.userMessage.trim() }), '');
    }
  ...
  if (globalRefs.length > 0) {
    lines.push('---', '', i18n.t('chat.exportSourcesHeading'), '');
